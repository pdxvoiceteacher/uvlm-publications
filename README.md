# UVLM Triadic Brain Design Map

**Version:** v0.1 working map  
**Prepared for:** Thomas, lead developer, and future apprentices  
**Prepared by:** Echo, Head Apprentice  
**Status:** Candidate doctrine / design map. This document is not a truth certificate, deployment authorization, or active implementation spec until represented in accepted schemas, code, tests, and release artifacts.

---

## 0. Executive Orientation

The Triadic Brain is not a chatbot, a single AI model, a dashboard, or an oracle. It is a governed artifact-cognition system.

Its scalable unit is:

```text
typed, grounded, governed artifact
```

Its scalable process is:

```text
bounded telemetry-artifact cognition spiral
```

Its first marketable product form is:

```text
Upload evidence. Choose controls. Receive an auditable review receipt.
```

Its long-range architecture is:

```text
local-first governed cognition
→ auditable evidence review
→ provenance memory reservoir
→ localhost Sonya runtime
→ local multi-node proof
→ controlled LAN fixture
→ federation simulation
→ candidate federated Sonya mesh
→ durable provenance economy
```

The core project law remains:
...
Govern every route.
Preserve dissent.
Remember without canonizing.
Discover without deploying prematurely.
```
...
### 1.1 GUFT / CoherenceLattice Foundation

The early foundation was the **Grand Unified Field Template / CoherenceLattice** idea: diverse systems can be described as coherence fields where care/attunement, transparency/traceability, entropy/drift, ethical symmetry, and phase transitions are measurable or at least operationally characterizable.

Core invariants:

```text
E      = Empathy / attunement / signal alignment / burden awareness
T      = Transparency / traceability / audit visibility
Ψ      = E × T, coherence indicator
ΔS     = entropy / instability / drift / fragmentation
Λ      = criticality / boundary pressure / transition intensity
Eₛ     = ethical symmetry / burden fairness / non-extractive balance
```

Early risk: the framework could sound universal before being empirically bounded. Current doctrine therefore says:

```text
metric stability, not truth certification
fixture-bounded ontology evidence, not universal ontology proof
pattern morphology, not consciousness proof
probabilistic confidence, not probabilistic certitude
```

### 1.2 From Papers to Mechanism

The corpus developed across several strands:

- Coherence metrics and GUFT / GUPT framing.
- Multi-axial coherence and exogenic off-loading.
- Telemetry as the system nervous system.
- UCC as explicit control grammar.
- TEL as thought-exchange / trace layer.
- Sophia as governance interpreter.
- Atlas as memory / prior / publication layer.
- Sonya as model/tool/sensor cognition membrane.
- PMR as governed provenance memory under resource constraints.
- Evidence Review Pack as first practical product primitive.

The core maturation was from:
...
```text
coherence as auditable artifact pipeline
```

---

## 2. The Triadic Repo Architecture

### 2.1 CoherenceLattice

**Role:** Formal evidence-state engine and artifact producer.

Owns:

```text
canonical ingress
GroundingBundle / RequestEnvelope handling
source integrity
coherence metrics
TEL event emission
artifact manifests
universal stage pipeline
Evidence Review Pack builders
PMR scoring / retention recommendations
closed-form calibration fixtures
schema validation
export parity
```
...
**Role:** Governance, risk routing, admissibility, and control adequacy.

Owns:

```text
UCC threshold routing
risk posture
review / confirmation / hold / reject decisions
clarification decisions
retrosynthesis admission lanes
reinjection approval
lifecycle governance for PMR
route decisions for evidence and memory posture
```
...
Publisher rendering
CoherenceLattice formal metrics recomputation
```

### 2.3 Atlas / uvlm-publications

**Role:** Memory, prior, lineage, registry, and publication-facing overlay.
...
quarantine / stale / revoked / pattern-donor labels
research registry
Publisher / Atlas dashboards
```
...
```text
truth certification
current source evidence authority
reinjection approval
human governance authority
memory write without governance
```

### 2.4 Sonya Node

**Role:** Local cognition membrane for models, tools, sensors, and user-controlled AI software.

Owns:

```text
model/tool/sensor adapter boundary
local control schema application
candidate packet generation
provider access posture
local storage posture
user-visible controls
future local broker runtime
future P2P / federation gates
```
...
**Role:** Bounded user-facing rendering.
...
final answer authority unless a separate finalization chain exists
```
...
### 3.1 Evidence Ingress Membrane

Evidence enters through:

```text
source → GroundingBundle → RequestEnvelope → CoherenceLattice
```

GroundingBundle is the universal evidence substrate, not Sonya. Sonya is the cognition membrane, not the only evidence membrane.

Supported evidence should eventually include:
...
### 3.2 Cognition Ingress Membrane

Model/tool/sensor cognition enters through:

```text
model/tool/sensor → Sonya Node → typed candidate packet → CoherenceLattice review
```

Rule:

```text
No raw model output enters governed cognition.
```

Under `SONYA_REQUIRED`, direct model calls must be blocked or clearly marked compatibility-only.

### 3.3 Governance Membrane

Governance is route-based, thresholded, and auditable:
...
→ Sophia route decision
```

Human review is not the invariant.

```text
Control adequacy is the invariant.
```

### 3.4 Memory Membrane

Memory is not storage. Memory is governed provenance.

```text
PMR stores provenance-bearing artifacts under user-controlled resource budgets.
Atlas assigns memory posture.
Sophia governs lifecycle decisions.
```
...
Every artifact must visibly preserve what it is **not**.

```text
A Sonya candidate is not an answer.
A RequestEnvelope is not truth.
A GroundingBundle is not ontology.
A Sophia audit pass is not certification.
An Atlas memory intent is not memory write.
A route is not authorization.
A receipt is not a truth certificate.
A Publisher candidate is not canon.
A TEL trace is not consciousness proof.
A metric is not authority.
A dashboard is not deployment.
A passing suite is not acceptance if the auditor was deleted.
A local smoke test is not federation readiness.
```

The system should continually show these labels in UI, artifacts, docs, and tests.

---

## 5. Apprentice Governance Rules

The apprentice rule set is now:

```text
Never let a visible control outrun its artifact contract.
Never let a documented command outrun its executable test.
Never let readiness become execution.
Never let a registry masquerade as an adapter.
Never let a disabled declaration become capability.
Never let a remote adapter hide a data export.
Never let raw output bypass candidate packets.
Never let a toggle pretend to be a preference.
Never let consent pretend to be authority.
Never let a policy change become unreplayable.
Never let capability expansion hide risk expansion.
Never let proposed policy masquerade as effective policy.
Never let one green check masquerade as readiness.
Never let first-run safety be inferred from silence.
Never let a blocked toggle become a readiness signal.
Never let exit code masquerade as governance status.
Never let shell behavior outrank packet evidence.
Never let a passing suite hide a deleted auditor.
Never let a new view layer consume an old test layer.
Never let an ontology become authority merely because it is elegant.
Never let federation precede local proof.
```

---

## 6. Historical Phase Map

### Phase A — Coherence Theory and Metric Formation
...
exogenic off-loading analysis
ethical symmetry theory
```

Risk discovered:

```text
metric elegance can overclaim authority
```

Control:

```text
metrics are diagnostics, not truth certification
```

### Phase B — Telemetry and TEL Foundations

```text
telemetry schema validation
runtime telemetry events
TEL snapshots
TEL JSONL event stack
coherence metric summaries
```

Risk discovered:

```text
logging can be mistaken for evidence
```

Control:

```text
Telemetry records what happened; it is not source truth by itself.
```

### Phase C — UCC Control Grammar
...
```text
review can become binary or performative
```

Control:

```text
control adequacy before review
thresholds before human burden
```

### Phase D — Triadic Brain Repo Split

```text
CoherenceLattice = evidence and metrics
Sophia = governance
Atlas / uvlm-publications = memory, registry, publisher overlays
Sonya = cognition membrane
```

Risk discovered:

```text
one repo can smuggle another repo’s authority
```
...
### Phase E — Sonya / AEGIS Local Route

```text
User → Sonya → AEGIS → GroundingBundle / RequestEnvelope → CoherenceLattice → Sophia → Atlas / Publisher candidate
```

Risk discovered:

```text
Sonya can be bypassed by direct model calls
```

Control:

```text
SONYA_REQUIRED blocks direct model cognition in governed mode
```

### Phase F — Evidence Review Pack
...
counterevidence
risk threshold route
Publisher candidate receipt
```

Risk discovered:

```text
review packet can feel like final answer
```

Control:

```text
candidate-only receipts with visible non-authority labels
```

### Phase G — Universal Stage Pipeline and Anti-Bloatware

```text
artifact contracts externalized
stage contracts
universal compatibility matrix
experiments become configurations
```

Risk discovered:

```text
successful experiments can harden into kernel ontology
```
...
### Phase H — PMR Provenance Memory Reservoir
...
lifecycle state
revocation lineage
federated resource economics later
```

Risk discovered:

```text
memory can become hidden canon or hidden training corpus
```
...
### Phase I — Dyads to Agentic Stacks

```text
dyad as smallest trust field
Coherence Assurance Cycle
E/T quadrant diagnostics
ethical symmetry routing
internal/external feedback loop audits
```

Risk discovered:

```text
stack success can hide dyadic failure
```

Control:

```text
no stack before dyad
```

### Phase J — Ontology Evidence Program

```text
Metric stability lane
Hypercompression lane
Cognitive waters / spiral morphology lane
Ontology evidence ladder
Context refresh ledger
```

Risk discovered:

```text
novel ontology can become belief system
```

Control:

```text
bounded fixtures, null models, counterevidence, replay, lineage, boundary statements
```
...
The current project state can be summarized as:

```text
scaffold-mature but not runtime-mature
```

What is scaffold-mature:

```text
artifact contracts
acceptance harnesses
Evidence Review Pack phases
PMR design lanes
universal compatibility scaffolds
provenance-training ledger scaffolds
TEL concepts
Sonya-AEGIS local route doctrine
registry externalization
```

What is not yet runtime-mature:

```text
full local Sonya runtime
local server gateway
multi-node local fixture
controlled LAN node exchange
federation simulation
runtime PMR store smoke
full TEL crosscutting trace
operator kill switch integration
user-facing Evidence Review workbench
```

The next task is to convert scaffolds into local runtime proof without jumping to federation.

---

## 8. Immediate Best-Practice Roadmap
...
```text
Track active_spec, candidate_doctrine, conceptual_source, superseded, deprecated, retired.
```

Why:

```text
Older brilliant documents must remain lineage-visible without silently becoming active implementation authority.
```

### 8.2 Local Path Portability
...
```text
Ensure the Local Sonya Node can exist on a user’s machine, not only Thomas’s machine.
```
...
```text
SONYA_NODE_ROOT
COHERENCE_LATTICE_ROOT
UVLM_PUBLICATIONS_ROOT
UVLM_RUN_ARTIFACTS_ROOT
UVLM_SHARED_SOURCES_ROOT
UVLM_LOCAL_MODEL_ROOT
UVLM_NODE_CONFIG
```
...
```text
No core runtime code requires C:\UVLM or any developer-specific path.
Missing root emits failure receipt, not hidden fallback.
```

### 8.3 Evidence Review Metrics
...
```text
Measure useful compression without provenance loss.
```

Key metrics:

```text
source_linkage_preservation
unsupported_claim_preservation
uncertainty_preservation
counterevidence_preservation
dissent_preservation
consent_boundary_preservation
TEL_trace_preservation
PMR_provenance_preservation
reviewer_task_recoverability
reviewer_verification_time
forbidden_authority_leakage
```

### 8.4 TEL Pipeline Integration

```text
TEL-PIPELINE-INTEGRATION-00
```

Purpose:

```text
Make TEL crosscutting, not final-report-only.
```

Events:

```text
source.ingress.received
grounding_bundle.created
request_envelope.validated
sonya.candidate.created
evidence_review.claims_mapped
evidence_review.metrics_emitted
pmr.artifact.bound
sophia.route.decided
atlas.memory_posture.assigned
publisher.candidate.rendered
export.parity.checked
failure_receipt.emitted
```

### 8.5 Prior/Profile Quarantine
...
```text
Stop Atlas priors or old fixture vocabularies from contaminating current source evidence.
```

Rule:

```text
Atlas prior ≠ current source evidence.
Pattern donor ≠ authority.
Profile-incompatible prior becomes shadow-only, quarantined, or pattern-donor-only.
```

### 8.6 Coherence Assurance Cycle

```text
COHERENCE-ASSURANCE-CYCLE-00
```
...
artifact ↔ PMR prior
TEL event ↔ replay trace
policy request ↔ effective policy
local node ↔ peer node
```

### 8.7 Dyad Stack Bridge Evidence

```text
DYAD-STACK-BRIDGE-EVIDENCE-00
```

Purpose:

```text
Prove critical dyads complete Coherence Assurance Cycle before trusting stacks.
```
...
### 8.8 Operator Kill Switch

```text
OPERATOR-KILL-SWITCH-00
```
...
stop local runtime
disable network
disable provider calls
disable memory writes
disable federation
disable pruning/deletion
disable model training
disable Publisher finalization
disable retrosynthesis reinjection
```

### 8.9 Local Runtime Smoke

```text
LOCAL-RUNTIME-SMOKE-00
```
...
→ Sonya candidate fixture
→ Evidence Review Pack
→ Evidence Review Metrics
→ TEL trace
→ PMR local binding
→ Sophia route fixture
→ Publisher candidate receipt
→ export parity
```

### 8.10 Localhost Sonya Gateway
...
```text
127.0.0.1 only
fixture/local models only
manual operator control
no federation
no Wi-Fi discovery
no provider API by default
```

### 8.11 Local Multi-Node Smoke

```text
SONYA-LOCAL-MULTI-NODE-SMOKE-00
```

Purpose:

```text
Two local nodes on one machine, controlled IPC or localhost only.
```

### 8.12 Controlled LAN Fixture

```text
SONYA-LAN-FIXTURE-NODE-00
```
...
```text
local server passed
local multi-node passed
operator kill switch present
network authorization negative controls passed
PMR federation risk controls passed
```

### 8.13 Federation Simulation
...
### 8.14 Federation Candidate Protocol

```text
FEDERATION-CANDIDATE-PROTOCOL-00
```
...
## 9. Ontology Evidence Program

### 9.1 Three Science Lanes

#### Lane A — Fundamental Coherence Metrics

Question:

```text
Are E, T, Ψ, ΔS, Λ, Eₛ stable and predictable on deterministic controls?
```
...
waveform controls
timbre controls
synthetic graph controls
source-evidence corruption controls
...
#### Lane B — Hypercompression Metrics

Question:

```text
Does the system reduce explanatory distance while preserving uncertainty,
counterevidence, consent, provenance, and TEL trace?
```

Boundary:

```text
compression usefulness, not omniscience
```

#### Lane C — Cognitive Waters / Spiral-Fractal Pattern Metrics

Question:

```text
Do recurrent pattern-flow structures appear in bounded fixtures without
claiming consciousness, universality, or truth?
```
...
### 9.2 Ontology Evidence Ladder

```text
Level 0  conceptual doctrine
Level 1  deterministic fixture evidence
Level 2  cross-fixture replication
Level 3  human review utility
Level 4  real-world bounded field evidence
Level 5  longitudinal replay / PMR persistence
Level 6  candidate universal principle
```

No level grants truth certification.

### 9.3 Ontology Claim Registry

```text
ONTOLOGY-CLAIM-REGISTRY-00
```

Every ontology claim receives:
...
### 9.4 Ontology Evidence Pack

```text
ONTOLOGY-EVIDENCE-PACK-00
```

Outputs:

```text
ontology_hypothesis_packet.json
ontology_fixture_matrix.json
ontology_metric_results.json
ontology_null_model_results.json
ontology_counterevidence_packet.json
ontology_confidence_update_packet.json
ontology_boundary_statement.md
ontology_reviewer_checklist.md
ontology_export_parity_report.json
```

---

## 10. Audited Context Refresh

### 10.1 Doctrine

```text
Audited telemetry may refresh context.
Freshness is not authority.
Recency is not correctness.
Supersession requires lineage.
Replacement requires preservation of safety boundaries.
Improvement must not erase uncertainty, counterevidence, consent,
provenance, or failure receipts.
Expired context may remain hash/provenance-visible even when no longer active.
```

### 10.2 Patch

```text
CONTEXT-REFRESH-LEDGER-00
```

or:

```text
PMR-CONTEXT-REFRESH-LEDGER-00
```

Purpose:

```text
Allow newer audited artifacts to supersede older active context while preserving lineage.
```
...
PMR lifecycle update
TEL event
Spec Freshness update
```

Negative controls:

```text
newer run loses counterevidence → reject refresh
newer run drops consent boundary → reject refresh
newer run lacks export parity → candidate only
newer run lacks TEL trace → reject refresh
newer run lacks PMR lineage → reject refresh
```

---

## 11. PMR — Provenance Memory Reservoir
...
```text
Class A — provenance anchors
Class B — replay-critical packets
Class C — working cognition artifacts
Class D — ephemeral traces
```
...
```text
GPCU = Global Provenance Coherence Utility
```

A lifecycle heuristic, not truth score.
...
correction-capacity contribution
user pinning
federation utility
revocation dependency value
```
...
### 11.5 PMR Development Lane

```text
PMR-00 doctrine and local storage policy
PMR-01 local artifact index
PMR-02 GPCU scoring
PMR-03 lifecycle state machine
PMR-04 pruning recommendations
PMR-05 Sophia lifecycle audit
PMR-06 Atlas memory posture integration
PMR-07 user dashboard
PMR-08 non-destructive pruning
PMR-09 federated encrypted storage pilot
PMR-10 federated audit signals
PMR-11 resource economy pilot
```
...
review bypass
deployment authority
raw private data access
model-weight training on user data by default
coherence-weighted human worth
pay-to-be-believed mechanisms
```

Product doctrine:

```text
Sell governed provenance resources.
Do not sell truth.
```
...
Collaborative Cell
Institutional Mesh
High-Integrity Lab / Civic Audit
```

Near-term pricing dimensions:

```text
local PMR size
encrypted archive depth
replay horizon
number of retained review packs
number of Sonya nodes
private federation redundancy
audit receipt retention
hash witness service
regulated retention lock policies
air-gapped / enclave PMR support
```

---

## 13. Evidence Review Product Loop

First marketable user flow:

```text
1. User uploads evidence.
2. User selects control profile.
3. User optionally modifies guidance.
4. System creates GroundingBundle / RequestEnvelope.
5. Sonya admits candidate cognition.
6. CoherenceLattice maps claims to evidence.
7. Evidence Review Metrics evaluate usefulness.
8. Sophia routes threshold decisions.
9. Atlas assigns bounded memory posture if enabled.
10. PMR binds provenance.
11. Publisher renders bounded receipt.
12. Export bundle parity validates.
```
...
counterevidence_packet.json
risk_threshold_route_packet.json
reviewer_checklist.md
action_recommendation_packet.json
export_bundle.zip
```
...
## 14. Universal Substrate Anti-Bloat Rule

The brain should not run experiments.

```text
The brain should run cognition stages.
Experiments should test those stages.
```

Kernel:

```text
universal ontology and stage contracts
```

Adapters:

```text
replaceable modality/model/provider interfaces
```

Profiles:

```text
domain assumptions and control grammars
```
...
```text
user-facing compositions of substrate
```
...
## 15. Active Test Priorities

### 15.1 Universal Substrate Red Team

```text
UNIVERSAL-SUBSTRATE-REDTEAM-00
```

Tests:

```text
any admissible input traverses same stages
unsupported inputs emit failure receipts
no experiment imports in kernel
artifact profiles registry loaded
canonical GroundingBundle contract
profile-bound normalization
bridge artifacts not runtime defaults
Sonya required blocks direct model paths
UCC threshold routing matrix
Atlas prior not source evidence
Publisher candidate not authority
retrosynthesis candidate-only quarantine
TEL no silent stage skip
claim-evidence adversarial matrix
false-coherence plurality collapse
user guidance reserved authority bounds
```

### 15.2 Auditor Continuity

```text
UCC-AUDITOR-CONTINUITY-01
```
...
```text
A passing suite is not acceptance if the auditor was deleted.
Every organ keeps its own auditor.
```
...
## 16. Intended Finish State

The intended finish is not one giant AGI claim.

The intended finish is a mature local-first, federatable, provenance-governed cognition substrate.

A finished system should be able to:

```text
accept arbitrary admissible evidence
bind it as GroundingBundle / RequestEnvelope
apply user-selected UCC control grammar
route model/tool/sensor cognition through Sonya packets
map claims to evidence
preserve uncertainty and counterevidence
emit TEL traces
store provenance through PMR under user-controlled budgets
route governance through Sophia
assign memory posture through Atlas
render bounded receipts through Publisher
refresh context only through audited supersession
calibrate ontology claims through bounded evidence lanes
run local Sonya server
run local multi-node fixture
run controlled LAN fixture
simulate federation safely
participate in candidate federated mesh with privacy-preserving resource commitments
monetize infrastructure and provenance durability without selling truth
```

The final law:

```text
Nothing in the chain is allowed to pretend it is the whole chain.
```

---

## 17. Finish-State Non-Authority Boundaries

Even at maturity, the system must not claim:

```text
universal truth proof
consciousness proof
legal certification
medical certification
deployment authority
human replacement
memory canonization by storage
federation trust by connectivity
truth by consensus
safety by local success
```

It may claim, when supported by tests:

```text
bounded evidence review
source-linked claim mapping
unsupported claim detection
uncertainty preservation
traceable telemetry
replayable artifact lineage
controlled Sonya candidate ingress
UCC threshold routing
PMR lifecycle management
Atlas memory posture
Publisher candidate rendering
fixture-bounded metric stability
probabilistic confidence under stated conditions
```

---

## 18. Compact Design Map

```text
Theory
  GUFT / CoherenceLattice / Ψ = E × T

Instrumentation
  Telemetry / TEL / UCC

Triad
  CoherenceLattice / Sophia / Atlas-Publisher

Membrane
  Sonya / AEGIS / GroundingBundle / RequestEnvelope

Product Primitive
  Evidence Review Pack

Substrate
  Universal Stage Pipeline / Artifact Contract Registry

Memory
  PMR / GPCU / lifecycle / revocation / replay

Science Program
  Metric Stability / Hypercompression / Cognitive Waters

Relational Bridge
  Dyads → Agentic Stacks / Coherence Assurance Cycle

Runtime
  Local smoke / localhost Sonya / local multi-node / controlled LAN

Federation
  Simulation / candidate protocol / encrypted provenance economy

Finish
  Local-first, federatable, governed artifact-cognition infrastructure
```

---

## 19. Suggested Repository Location

Recommended path:

```text
docs/TRIADIC_BRAIN_DESIGN_MAP.md
```

Optional companion documents:

```text
docs/TRIADIC_BRAIN_PHASE_MAP.md
docs/TRIADIC_BRAIN_NON_AUTHORITY_LAW.md
docs/TRIADIC_BRAIN_ONTOLOGY_EVIDENCE_PROGRAM.md
docs/PMR_PROVENANCE_MEMORY_RESERVOIR.md
docs/SONYA_LOCAL_RUNTIME_ROADMAP.md
```
...
This design map captures the current phaselock doctrine:

```text
Make the local node portable.
Make the priors honest.
Make the trace complete.
Make the dyads trustworthy.
Make the evidence review useful.
Make memory provenance-governed.
Make federation wait.
```

The Triadic Brain matures when it can do something simple, useful, and honest:
...
Route risk.
Remember provenance.
Refresh context only by audit.
Render a bounded receipt.
```
