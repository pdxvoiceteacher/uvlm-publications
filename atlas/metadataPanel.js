...
function renderConcept(data) {
  return [
    ...
    row('Altruistic Corridor Score', Number(data.altruisticCorridorScore ?? 0).toFixed(2)),
    row('Consent / Feedback Friction', data.consentFeedbackFriction ?? 'bounded'),
    row('Multiscale Support Class', data.multiscaleSupportClass ?? 'bounded'),
    row('Conformity Penalty', Number(data.conformityPenalty ?? 0).toFixed(2)),
    row('Repair Corridor Flag', data.repairCorridorFlag === true ? 'true' : 'false'),
    row('Distortion Risk Class', data.distortionRiskClass ?? 'bounded'),
    row('River Seed Potential', data.riverSeedPotential ?? 'bounded'),
    row('River Formation Signal', data.riverFormationSignal ?? 'emergent'),
    row('Corridor Weaving Score', Number(data.corridorWeavingScore ?? 0).toFixed(2)),
