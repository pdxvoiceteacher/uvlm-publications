...
export async function loadAttentionOverlay() {
  const [
    ...
    operationalMaturityDashboardResp,
    deploymentBoundaryRegistryResp,
    translationRiskWatchlistResp,
    operationalizationAnnotationsResp,
    discoveryNavigationDashboardResp,
    discoveryCorridorRegistryResp,
    discoveryRiskWatchlistResp,
    discoveryAnnotationsResp
  ] = await Promise.all([
    ...
    fetch('../registry/operational_maturity_dashboard.json').catch(() => null),
    fetch('../registry/deployment_boundary_registry.json').catch(() => null),
    fetch('../registry/translation_risk_watchlist.json').catch(() => null),
    fetch('../registry/operationalization_annotations.json').catch(() => null),
    fetch('../registry/discovery_navigation_dashboard.json').catch(() => null),
    fetch('../registry/discovery_corridor_registry.json').catch(() => null),
    fetch('../registry/discovery_risk_watchlist.json').catch(() => null),
    fetch('../registry/discovery_annotations.json').catch(() => null)
  ]);
  ...
  return {
    ...
    operationalMaturityDashboard: operationalMaturityDashboardResp ? await operationalMaturityDashboardResp.json() : {},
    deploymentBoundaryRegistry: deploymentBoundaryRegistryResp ? await deploymentBoundaryRegistryResp.json() : {},
    translationRiskWatchlist: translationRiskWatchlistResp ? await translationRiskWatchlistResp.json() : {},
    operationalizationAnnotations: operationalizationAnnotationsResp ? await operationalizationAnnotationsResp.json() : {},
    discoveryNavigationDashboard: discoveryNavigationDashboardResp ? await discoveryNavigationDashboardResp.json() : {},
    discoveryCorridorRegistry: discoveryCorridorRegistryResp ? await discoveryCorridorRegistryResp.json() : {},
    discoveryRiskWatchlist: discoveryRiskWatchlistResp ? await discoveryRiskWatchlistResp.json() : {},
    discoveryAnnotations: discoveryAnnotationsResp ? await discoveryAnnotationsResp.json() : {}
  };
}
...
function buildCivilizationalMemoryConceptSignals(civilizationalMemoryDashboard, epistemicResilienceRegistry, memoryFragilityWatchlist, civilizationalMemoryAnnotations) {
  const byConcept = new Map();
  ...
  function bump(targetId, update) {
    ...
    const existing = byConcept.get(targetId) ?? {
      memoryStatus: 'monitor',
      preservationCriticality: 'routine',
      legibilityPersistence: 'bounded',
      vocabularyDriftRisk: 'bounded',
      notationFragility: 'bounded',
      recoverability: 'bounded',
      custodyDiversity: 'mixed',
      memoryQueueStatus: 'none',
    };
    update(existing);
    byConcept.set(targetId, existing);
  }
  ...
  const registryByReview = new Map();
  asArray(epistemicResilienceRegistry?.entries).forEach((entry) => {
    if (typeof entry?.reviewId === 'string') {
      registryByReview.set(entry.reviewId, entry);
    }
  });

  asArray(civilizationalMemoryDashboard?.entries).forEach((entry) => {
    const reg = registryByReview.get(entry?.reviewId);
    asArray(entry?.linkedTargetIds).forEach((targetId) => {
      bump(targetId, (s) => {
        s.memoryStatus = entry?.memoryStatus ?? reg?.memoryStatus ?? s.memoryStatus;
        s.preservationCriticality = entry?.preservationCriticality ?? reg?.preservationCriticality ?? s.preservationCriticality;
        s.legibilityPersistence = entry?.legibilityPersistence ?? reg?.legibilityPersistence ?? s.legibilityPersistence;
        s.vocabularyDriftRisk = entry?.vocabularyDriftRisk ?? reg?.vocabularyDriftRisk ?? s.vocabularyDriftRisk;
        s.notationFragility = entry?.notationFragility ?? reg?.notationFragility ?? s.notationFragility;
        s.recoverability = entry?.recoverability ?? reg?.recoverability ?? s.recoverability;
        s.custodyDiversity = entry?.custodyDiversity ?? reg?.custodyDiversity ?? s.custodyDiversity;
        s.memoryQueueStatus = 'dashboard';
      });
    });
  });

  asArray(memoryFragilityWatchlist?.entries).forEach((entry) => {
    asArray(entry?.linkedTargetIds).forEach((targetId) => {
      bump(targetId, (s) => {
        s.memoryStatus = entry?.memoryStatus ?? s.memoryStatus;
        s.preservationCriticality = entry?.preservationCriticality ?? s.preservationCriticality;
        s.legibilityPersistence = entry?.legibilityPersistence ?? s.legibilityPersistence;
        s.vocabularyDriftRisk = entry?.vocabularyDriftRisk ?? s.vocabularyDriftRisk;
        ...
        if (s.memoryQueueStatus !== 'dashboard') {
          s.memoryQueueStatus = 'watch';
        }
      });
    });
  });

  asArray(civilizationalMemoryAnnotations?.annotations).forEach((entry) => {
    asArray(entry?.linkedTargetIds).forEach((targetId) => {
      bump(targetId, (s) => {
        s.memoryStatus = entry?.memoryStatus ?? s.memoryStatus;
        s.preservationCriticality = entry?.preservationCriticality ?? s.preservationCriticality;
...
function buildOperationalizationConceptSignals(
  operationalMaturityDashboard,
  deploymentBoundaryRegistry,
  translationRiskWatchlist,
  operationalizationAnnotations
) {
  const byConcept = new Map();
  ...
  function bump(targetId, update) {
    ...
    const existing = byConcept.get(targetId) ?? {
      ...
      maturityClass: 'bounded',
      deploymentReadiness: 'bounded',
      deadZoneAdjacency: 'bounded',
      translationRisk: 'bounded',
      requiredSafeguards: [],
      commonsReviewRequirement: 'required',
      readinessScore: 0,
      translationRiskScore: 0,
      operationalQueueStatus: 'none'
    };
    update(existing);
    byConcept.set(targetId, existing);
  }

  const boundaryByReview = new Map();
  asArray(deploymentBoundaryRegistry?.entries).forEach((entry) => {
    if (typeof entry?.reviewId === 'string') {
      boundaryByReview.set(entry.reviewId, entry);
    }
  });

  asArray(operationalMaturityDashboard?.entries).forEach((entry) => {
    const bound = boundaryByReview.get(entry?.reviewId);
    asArray(entry?.linkedTargetIds).forEach((targetId) => {
      bump(targetId, (s) => {
        s.operationalStatus = entry?.operationalStatus ?? bound?.operationalStatus ?? s.operationalStatus;
        s.maturityClass = entry?.maturityClass ?? bound?.maturityClass ?? s.maturityClass;
        s.deploymentReadiness = bound?.deploymentReadiness ?? entry?.deploymentReadiness ?? s.deploymentReadiness;
        s.deadZoneAdjacency = bound?.deadZoneAdjacency ?? entry?.deadZoneAdjacency ?? s.deadZoneAdjacency;
        s.translationRisk = entry?.translationRisk ?? bound?.translationRisk ?? s.translationRisk;
        s.requiredSafeguards = asArray(entry?.requiredSafeguards?.length ? entry?.requiredSafeguards : bound?.requiredSafeguards);
        s.commonsReviewRequirement = entry?.commonsReviewRequirement ?? bound?.commonsReviewRequirement ?? s.commonsReviewRequirement;
        s.readinessScore = Number(entry?.readinessScore ?? bound?.readinessScore ?? s.readinessScore);
        s.translationRiskScore = Number(entry?.translationRiskScore ?? bound?.translationRiskScore ?? s.translationRiskScore);
        s.operationalQueueStatus = 'dashboard';
      });
    });
  });

  asArray(translationRiskWatchlist?.entries).forEach((entry) => {
    asArray(entry?.linkedTargetIds).forEach((targetId) => {
      bump(targetId, (s) => {
        s.translationRisk = entry?.translationRisk ?? s.translationRisk;
        s.translationRiskScore = Number(entry?.translationRiskScore ?? s.translationRiskScore);
        s.requiredSafeguards = asArray(entry?.requiredSafeguards?.length ? entry?.requiredSafeguards : s.requiredSafeguards);
        s.commonsReviewRequirement = entry?.commonsReviewRequirement ?? s.commonsReviewRequirement;
        if (s.operationalQueueStatus !== 'dashboard') {
          s.operationalQueueStatus = 'watch';
        }
      });
    });
  });

  asArray(operationalizationAnnotations?.annotations).forEach((entry) => {
    asArray(entry?.linkedTargetIds).forEach((targetId) => {
      bump(targetId, (s) => {
        s.operationalStatus = entry?.operationalStatus ?? s.operationalStatus;
        s.maturityClass = entry?.maturityClass ?? s.maturityClass;
        s.deploymentReadiness = entry?.deploymentReadiness ?? s.deploymentReadiness;
...
function buildDiscoveryNavigationConceptSignals(
  discoveryNavigationDashboard,
  discoveryCorridorRegistry,
  discoveryRiskWatchlist,
  ...
  function bump(targetId, update) {
    ...
    const existing = byConcept.get(targetId) ?? {
      discoveryStatus: 'monitor',
      vectorClass: 'bounded',
      bridgeMaturity: 'bounded',
      corridorClass: 'bounded',
      deadZoneAdjacency: 'bounded',
  ...
  const corridorByReview = new Map();
  asArray(discoveryCorridorRegistry?.entries).forEach((entry) => {
    if (typeof entry?.reviewId === 'string') {
      corridorByReview.set(entry.reviewId, entry);
    }
  });

  asArray(discoveryNavigationDashboard?.entries).forEach((entry) => {
    const corridor = corridorByReview.get(entry?.reviewId);
    asArray(entry?.linkedTargetIds).forEach((targetId) => {
      bump(targetId, (s) => {
        s.discoveryStatus = entry?.discoveryStatus ?? corridor?.discoveryStatus ?? s.discoveryStatus;
        s.vectorClass = entry?.vectorClass ?? corridor?.vectorClass ?? s.vectorClass;
        s.bridgeMaturity = entry?.bridgeMaturity ?? corridor?.bridgeMaturity ?? s.bridgeMaturity;
        s.corridorClass = corridor?.corridorClass ?? entry?.corridorClass ?? s.corridorClass;
  ...
  asArray(discoveryRiskWatchlist?.entries).forEach((entry) => {
    asArray(entry?.linkedTargetIds).forEach((targetId) => {
      bump(targetId, (s) => {
        s.discoveryStatus = entry?.discoveryStatus ?? s.discoveryStatus;
        s.vectorClass = entry?.vectorClass ?? s.vectorClass;
        s.bridgeMaturity = entry?.bridgeMaturity ?? s.bridgeMaturity;
        s.corridorClass = entry?.corridorClass ?? s.corridorClass;
        s.deadZoneAdjacency = entry?.deadZoneAdjacency ?? s.deadZoneAdjacency;
        ...
        if (s.discoveryQueueStatus !== 'dashboard') {
          s.discoveryQueueStatus = 'watch';
        }
      });
    });
  });

  asArray(discoveryAnnotations?.annotations).forEach((entry) => {
    asArray(entry?.linkedTargetIds).forEach((targetId) => {
      bump(targetId, (s) => {
        s.discoveryStatus = entry?.discoveryStatus ?? s.discoveryStatus;
        s.vectorClass = entry?.vectorClass ?? s.vectorClass;
...
export function applyAttentionOverlay(cy, overlay) {
  ...
  const civilizationalMemorySignals = buildCivilizationalMemoryConceptSignals(
    overlay?.civilizationalMemoryDashboard,
    overlay?.epistemicResilienceRegistry,
    overlay?.memoryFragilityWatchlist,
    overlay?.civilizationalMemoryAnnotations
  );
  const operationalizationSignals = buildOperationalizationConceptSignals(
    overlay?.operationalMaturityDashboard,
    overlay?.deploymentBoundaryRegistry,
    overlay?.translationRiskWatchlist,
    overlay?.operationalizationAnnotations
  );
  const discoveryNavigationSignals = buildDiscoveryNavigationConceptSignals(
    overlay?.discoveryNavigationDashboard,
    overlay?.discoveryCorridorRegistry,
    overlay?.discoveryRiskWatchlist,
    overlay?.discoveryAnnotations
  );
  ...
  const civilizationalMemoryProvenance = overlay?.civilizationalMemoryDashboard?.provenance ?? {};
  const civilizationalMemorySchemaVersion = civilizationalMemoryProvenance?.schemaVersions?.civilizational_memory_audit
    ?? civilizationalMemoryProvenance?.schemaVersions?.civilizational_memory_map
    ?? 'unknown';
  const civilizationalMemoryProducerCommits = asArray(civilizationalMemoryProvenance?.producerCommits).join(', ') || 'unknown';
  const civilizationalMemorySourceMode = civilizationalMemoryProvenance?.derivedFromFixtures ? 'fixture' : 'live';
  const operationalizationProvenance = overlay?.operationalMaturityDashboard?.provenance ?? {};
  const operationalizationSchemaVersion = operationalizationProvenance?.schemaVersions?.operational_maturity_map
    ?? operationalizationProvenance?.schemaVersions?.deployment_boundary_report
    ?? 'unknown';
  const operationalizationProducerCommits = asArray(operationalizationProvenance?.producerCommits).join(', ') || 'unknown';
  const operationalizationSourceMode = operationalizationProvenance?.derivedFromFixtures ? 'fixture' : 'live';
  const discoveryNavigationProvenance = overlay?.discoveryNavigationDashboard?.provenance ?? {};
  const discoveryNavigationSchemaVersion = discoveryNavigationProvenance?.schemaVersions?.discovery_vector_field
    ?? discoveryNavigationProvenance?.schemaVersions?.entropy_reduction_corridor
    ?? 'unknown';
  const discoveryNavigationProducerCommits = asArray(discoveryNavigationProvenance?.producerCommits).join(', ') || 'unknown';
  const discoveryNavigationSourceMode = discoveryNavigationProvenance?.derivedFromFixtures ? 'fixture' : 'live';
  ...
  cy.nodes('[class = "concept"]').forEach((node) => {
    ...
    node.data('canonicalIntegrityVerified', Boolean(overlay?.emergentDomainDashboard?.canonicalIntegrityVerified));
    node.data('modificationDisclosureMissing', Boolean(overlay?.emergentDomainDashboard?.modificationDisclosureMissing));
    node.data('trustPresentationDegraded', Boolean(overlay?.emergentDomainDashboard?.trustPresentationDegraded));
    node.data('originProject', overlay?.emergentDomainDashboard?.originProject ?? 'unknown');
    node.data('canonicalPhaselock', overlay?.emergentDomainDashboard?.canonicalPhaselock ?? 'unknown');
    node.data('ethicalBoundaryNotice', overlay?.emergentDomainDashboard?.ethicalBoundaryNotice ?? 'unknown');
    ...
    const civilizationalMemory = civilizationalMemorySignals.get(id);
    node.data('memoryStatus', civilizationalMemory?.memoryStatus ?? 'monitor');
    node.data('preservationCriticality', civilizationalMemory?.preservationCriticality ?? 'routine');
    node.data('legibilityPersistence', civilizationalMemory?.legibilityPersistence ?? 'bounded');
    node.data('vocabularyDriftRisk', civilizationalMemory?.vocabularyDriftRisk ?? 'bounded');
    node.data('notationFragility', civilizationalMemory?.notationFragility ?? 'bounded');
    node.data('memoryRecoverability', civilizationalMemory?.recoverability ?? 'bounded');
    node.data('custodyDiversity', civilizationalMemory?.custodyDiversity ?? 'mixed');
    node.data('civilizationalMemorySchemaVersion', civilizationalMemorySchemaVersion);
    node.data('civilizationalMemoryProducerCommits', civilizationalMemoryProducerCommits);
    node.data('civilizationalMemorySourceMode', civilizationalMemorySourceMode);
    const operationalization = operationalizationSignals.get(id);
    node.data('operationalStatus', operationalization?.operationalStatus ?? 'monitor');
    node.data('maturityClass', operationalization?.maturityClass ?? 'bounded');
    node.data('deploymentReadiness', operationalization?.deploymentReadiness ?? 'bounded');
    node.data('deadZoneAdjacency', operationalization?.deadZoneAdjacency ?? 'bounded');
    node.data('translationRisk', operationalization?.translationRisk ?? 'bounded');
    node.data('requiredSafeguards', asArray(operationalization?.requiredSafeguards));
    node.data('commonsReviewRequirement', operationalization?.commonsReviewRequirement ?? 'required');
    node.data('operationalReadinessScore', Number(operationalization?.readinessScore ?? 0));
    node.data('translationRiskScore', Number(operationalization?.translationRiskScore ?? 0));
    node.data('operationalizationSchemaVersion', operationalizationSchemaVersion);
    node.data('operationalizationProducerCommits', operationalizationProducerCommits);
    node.data('operationalizationSourceMode', operationalizationSourceMode);
    const discoveryNavigation = discoveryNavigationSignals.get(id);
    node.data('discoveryStatus', discoveryNavigation?.discoveryStatus ?? 'monitor');
    node.data('discoveryVectorClass', discoveryNavigation?.vectorClass ?? 'bounded');
    node.data('discoveryBridgeMaturity', discoveryNavigation?.bridgeMaturity ?? 'bounded');
    node.data('discoveryCorridorClass', discoveryNavigation?.corridorClass ?? 'bounded');
    node.data('discoveryDeadZoneAdjacency', discoveryNavigation?.deadZoneAdjacency ?? 'bounded');
    node.data('discoveryMemorySupport', discoveryNavigation?.memorySupport ?? 'bounded');
    ...
    if (['dashboard', 'watch'].includes(civilizationalMemory?.memoryQueueStatus ?? 'none')) {
      node.addClass('civilizational-memory-active');
    }
    if ((civilizationalMemory?.preservationCriticality ?? 'routine') === 'high') {
      node.addClass('preservation-criticality-high');
    }
    if ((civilizationalMemory?.legibilityPersistence ?? 'bounded') === 'drifting') {
      node.addClass('legibility-drifting');
    }
    if ((civilizationalMemory?.vocabularyDriftRisk ?? 'bounded') === 'high') {
      node.addClass('vocabulary-drift-risk-high');
    }
    if ((civilizationalMemory?.notationFragility ?? 'bounded') === 'fragile') {
      node.addClass('notation-fragility-high');
    }
    if ((civilizationalMemory?.recoverability ?? 'bounded') === 'strong') {
      node.addClass('memory-recoverability-strong');
    }
    if ((civilizationalMemory?.custodyDiversity ?? 'mixed') === 'high') {
      node.addClass('custody-diversity-high');
    }

    if (['dashboard', 'watch'].includes(operationalization?.operationalQueueStatus ?? 'none')) {
      node.addClass('operationalization-active');
    }
    if ((operationalization?.operationalStatus ?? 'monitor') === 'bounded-ready') {
      node.addClass('operational-status-bounded-ready');
    }
    if ((operationalization?.maturityClass ?? 'bounded') === 'field-tested') {
      node.addClass('maturity-class-field-tested');
    }
    if ((operationalization?.deploymentReadiness ?? 'bounded') === 'bounded') {
      node.addClass('deployment-readiness-bounded');
    }
    if ((operationalization?.deadZoneAdjacency ?? 'bounded') !== 'bounded') {
      node.addClass('dead-zone-adjacent');
    }
    if ((operationalization?.translationRisk ?? 'bounded') === 'high' || (operationalization?.translationRiskScore ?? 0) >= 0.6) {
      node.addClass('translation-risk-watch');
    }
    if (asArray(operationalization?.requiredSafeguards).length > 0) {
      node.addClass('safeguards-required');
    }
    if ((operationalization?.commonsReviewRequirement ?? 'required') === 'required') {
      node.addClass('commons-review-required');
    }

    if (['dashboard', 'watch'].includes(discoveryNavigation?.discoveryQueueStatus ?? 'none')) {
      node.addClass('discovery-navigation-active');
    }
    if ((discoveryNavigation?.discoveryStatus ?? 'monitor') === 'active') {
      node.addClass('discovery-status-active');
    }
    if ((discoveryNavigation?.vectorClass ?? 'bounded') === 'cross-domain') {
      node.addClass('discovery-vector-cross-domain');
    }
    if ((discoveryNavigation?.bridgeMaturity ?? 'bounded') === 'emergent') {
      node.addClass('discovery-bridge-emergent');
    }
    if ((discoveryNavigation?.corridorClass ?? 'bounded') === 'bounded') {
      node.addClass('discovery-corridor-bounded');
    }
    if ((discoveryNavigation?.deadZoneAdjacency ?? 'bounded') !== 'bounded') {
      node.addClass('discovery-dead-zone-adjacent');
    }
    if ((discoveryNavigation?.memorySupport ?? 'bounded') === 'supported') {
      node.addClass('discovery-memory-supported');
    }
    if ((discoveryNavigation?.commonsReviewRequirement ?? 'required') === 'required') {
