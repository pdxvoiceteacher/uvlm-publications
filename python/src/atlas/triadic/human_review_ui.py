...
def create_app(run_root,decision_root=None,explanation_path=None):
 ...
 async def commit(req:Request):
  ...
  try:
   ...
   did=str(uuid.uuid4());packet={'schema_id':'uvlm.human_review_decision.v1','schema_version':'1.0','packet_type':'human_review_decision','decision_id':did,'run_id':record['run_id'],'logical_time':record['logical_time'],'generated_at_utc':datetime.now(timezone.utc).isoformat(),'reviewer':{'display_name':record['reviewer'],'identity_assurance':'local_assertion_only','cryptographic_signature_present':False},'decision':record['decision'],'decision_note':record['note'],'source':{'interface':'atlas_local_human_review_ui','loopback_only':True},'evidence_bindings':record['evidence_bindings'],**({'explanation_evidence_bindings':record['explanation_evidence_bindings']} if record['explanation_evidence_bindings'] else {}),'sophia_disposition':record['sophia_disposition'],'atlas_retention_posture':record['atlas_retention_posture'],'atlas_publication_posture':record['atlas_publication_posture'],'requires_human_review':True,'authority_boundary':dict.fromkeys(AUTH,False),'side_effects':dict.fromkeys(EFFECTS,False),'nonauthority':'This receipt records a bounded human decision only.'};data=json.dumps(packet,sort_keys=True,separators=(',',':'),ensure_ascii=False).encode()+b'\n'
