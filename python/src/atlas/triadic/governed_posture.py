...
def assign_governed_posture(run_root: str|Path) -> dict[str,Any]:
    ...
    if v2:
        ...
        if req["provider_id"]!=provider["provider_id"] or req["provider_profile_sha256"]!=provider["profile_sha256"] or req["provider_trust_class"]!=provider["trust_class"] or req["data_egress"]!=provider["data_egress"] or not isinstance(req["external_provider_consent"],bool) or (provider["data_egress"]!="none" and req["external_provider_consent"] is not True): raise GovernedPostureError("v2 request/provider mismatch")
