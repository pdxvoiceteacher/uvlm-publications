    ...
    for phrase in FORBIDDEN_PHRASES:
        ...
        while True:
            ...
            if _is_blocked_overclaim_example_context(text, index):
                ...
                continue
            normalized_ces_pmr_allowed = _normalize(CES_PMR_INDEXING_CLAIM_ALLOWED)
            ces_pmr_allowed_start = text.find(normalized_ces_pmr_allowed)
            inside_ces_pmr_allowed_claim = False
            while ces_pmr_allowed_start != -1:
                if ces_pmr_allowed_start <= index <= ces_pmr_allowed_start + len(normalized_ces_pmr_allowed):
                    inside_ces_pmr_allowed_claim = True
                    break
                ces_pmr_allowed_start = text.find(normalized_ces_pmr_allowed, ces_pmr_allowed_start + 1)
            if inside_ces_pmr_allowed_claim:
                start = index + len(normalized_phrase)
                continue
...
def validate_dashboard(dashboard_path: Path, docs_dir: Path) -> dict[str, Any]:
    ...
    forbidden = [phrase for phrase in forbidden if phrase != "final answer authority"]
    truthy_bad_flags = [
        key
