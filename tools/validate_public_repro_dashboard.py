    ...
    for phrase in FORBIDDEN_PHRASES:
        ...
        while True:
            ...
            if phrase == "truth certified" and (
                ...
                or "safe excerpt means " in text[max(0, index - 48) : index]
                or "clear means " in text[max(0, index - 32) : index]
                or "candidate_allowed means " in text[max(0, index - 48) : index]
            ):
                start = index + len(normalized_phrase)
                continue
            if phrase == "audit action" and (
                "not audit action" in text[max(0, index - 64) : index + len(normalized_phrase)]
                or "claims_blocked" in text[max(0, index - 500) : index]
                or "passes " in text[max(0, index - 8) : index]
            ):
                start = index + len(normalized_phrase)
                continue
            if phrase == "adapter authorized" and (
                "local runtime enforcement adapter authorized_for" in text[max(0, index - 128) : index + len(normalized_phrase) + 80]
                or "adapter authorized_for" in text[max(0, index - 32) : index + len(normalized_phrase) + 80]
            ):
                start = index + len(normalized_phrase)
                continue
