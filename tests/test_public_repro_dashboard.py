...
from tools.build_public_repro_dashboard import (
    ...
    AEGIS_SOURCE_SCOPE_STATUSES,
    AEGIS_CONSENT_STATUSES,
    AEGIS_SOURCE_SCOPE_SCENARIOS,
    AEGIS_CONSENT_SCENARIOS,
    AEGIS_SOURCE_SCOPE_CONSENT_REPRO_FRAGMENTS,
    AEGIS_SOURCE_SCOPE_CONSENT_DOCTRINE,
    AEGIS_SOURCE_SCOPE_CONSENT_GUARDRAILS,
    AEGIS_SOURCE_SCOPE_CONSENT_SUMMARY,
    ...
    AEGIS_GROUNDING_SUMMARY,
    AEGIS_GROUNDING_BLOCKED_CLAIMS,
    AEGIS_GROUNDING_CLAIM_ALLOWED,
    AEGIS_GROUNDING_PRIOR_PHASE_RELATION,
    AEGIS_INSTRUCTION_QUARANTINE_ARTIFACTS,
    AEGIS_INSTRUCTION_QUARANTINE_STATUSES,
    AEGIS_INSTRUCTION_QUARANTINE_DECISIONS,
    AEGIS_INSTRUCTION_PATTERN_CLASSES,
    AEGIS_INSTRUCTION_QUARANTINE_SCENARIOS,
    AEGIS_INSTRUCTION_QUARANTINE_REPRO_FRAGMENTS,
    AEGIS_INSTRUCTION_QUARANTINE_DOCTRINE,
    AEGIS_INSTRUCTION_QUARANTINE_COMPATIBILITY,
    AEGIS_INSTRUCTION_QUARANTINE_GUARDRAILS,
    AEGIS_INSTRUCTION_QUARANTINE_SUMMARY,
    ...
    AEGIS_ACTION_FIREWALL_ARTIFACTS,
    AEGIS_ACTION_FIREWALL_STATUSES,
    AEGIS_ACTION_FIREWALL_DECISIONS,
    AEGIS_ACTION_KINDS,
    AEGIS_ACTION_FIREWALL_SCENARIOS,
    AEGIS_ACTION_FIREWALL_REPRO_FRAGMENTS,
    AEGIS_ACTION_FIREWALL_DOCTRINE,
    AEGIS_ACTION_FIREWALL_COMPATIBILITY,
    AEGIS_ACTION_FIREWALL_GUARDRAILS,
    AEGIS_ACTION_FIREWALL_SUMMARY,
    ...
    AEGIS_UI_PREFLIGHT_SOURCE_STATUSES,
    AEGIS_UI_PREFLIGHT_SURFACE_STATUSES,
    AEGIS_UI_PREFLIGHT_RENDERING_OUTCOMES,
    AEGIS_UI_PREFLIGHT_STATUS_MAPPINGS,
    AEGIS_UI_PREFLIGHT_SCENARIOS,
    AEGIS_UI_PREFLIGHT_REPRO_FRAGMENTS,
    AEGIS_UI_PREFLIGHT_ACCESSIBILITY_LANGUAGE,
    AEGIS_UI_PREFLIGHT_SECURITY_LANGUAGE,
    AEGIS_UI_PREFLIGHT_DOCTRINE,
    AEGIS_UI_PREFLIGHT_SOURCE_INTEGRITY_LANGUAGE,
    AEGIS_UI_PREFLIGHT_HASH_LANGUAGE,
    AEGIS_UI_PREFLIGHT_GUARDRAILS,
    AEGIS_UI_PREFLIGHT_SUMMARY,
    AEGIS_UI_PREFLIGHT_BLOCKED_CLAIMS,
    AEGIS_UI_PREFLIGHT_CLAIM_ALLOWED,
    AEGIS_UI_PREFLIGHT_PRIOR_PHASE_RELATION,
...
def _validator_reported_claim(forbidden_found: list[str], claim: str) -> bool:
    """Return true when the validator reports the injected claim or a meaningful matched span.

    The validator can report a shorter forbidden phrase than the full injected sentence.
    For example, injecting "claims memory write authorization" can validly return
    "claims memory write" when the shorter forbidden phrase is what the scanner matched.
    """
    claim_lower = claim.lower()
    expected_variants = (
        claim_lower,
        f"claims {claim_lower}",
    )

    for found in forbidden_found:
        found_lower = found.lower()
        if found_lower in expected_variants:
            return True

        for expected in expected_variants:
            if len(found_lower) >= 10 and expected.startswith(found_lower):
                return True
            if len(expected) >= 10 and found_lower.startswith(expected):
                return True
...
def test_validator_fails_if_rw_comp_03_makes_forbidden_claims(tmp_path):
    ...
    for claim in forbidden_claims:
        ...
        assert result["passed"] is False, claim
        forbidden_found = [found.lower() for found in result["forbidden_claims_found"]]
        assert _validator_reported_claim(forbidden_found, claim), result
...
def test_aegis_admission_and_risk_taxonomy_stack_publication_sync():
    ...
    pages = "\n".join(
        ...
        "docs/experiment-suite/aegis-ui-preflight-status-surface.md",
    ...
    artifact_expectations = {
        ...
        "AEGIS-UI-PREFLIGHT-STATUS-SURFACE-00": AEGIS_UI_PREFLIGHT_STATUS_SURFACE_ARTIFACTS,
    ...
    for key, value in AEGIS_SOURCE_SCOPE_CONSENT_SUMMARY.items():
        assert dashboard[f"aegis_source_scope_consent_{key}"] == value
    for key, value in AEGIS_GROUNDING_SUMMARY.items():
    ...
    for key, value in AEGIS_UI_PREFLIGHT_SUMMARY.items():
        assert dashboard[f"aegis_ui_preflight_status_surface_{key}"] == value
    ...
    for phrase_group in (
        ...
        AEGIS_RISK_PRIOR_PHASE_RELATION, AEGIS_SOURCE_SCOPE_CONSENT_DECISIONS,
        AEGIS_SOURCE_SCOPE_STATUSES, AEGIS_CONSENT_STATUSES,
        AEGIS_SOURCE_SCOPE_SCENARIOS, AEGIS_CONSENT_SCENARIOS,
        AEGIS_SOURCE_SCOPE_CONSENT_REPRO_FRAGMENTS, AEGIS_SOURCE_SCOPE_CONSENT_DOCTRINE,
        AEGIS_SOURCE_SCOPE_CONSENT_GUARDRAILS, AEGIS_SOURCE_SCOPE_CONSENT_BLOCKED_CLAIMS,
        AEGIS_SOURCE_SCOPE_CONSENT_PRIOR_PHASE_RELATION, AEGIS_GROUNDING_STATUSES,
        AEGIS_GROUNDING_DECISIONS, AEGIS_GROUNDING_SCENARIOS, AEGIS_GROUNDING_REPRO_FRAGMENTS,
        AEGIS_GROUNDING_DOCTRINE, AEGIS_GROUNDING_COMPATIBILITY, AEGIS_GROUNDING_GUARDRAILS,
        AEGIS_GROUNDING_BLOCKED_CLAIMS, AEGIS_GROUNDING_PRIOR_PHASE_RELATION,
        AEGIS_INSTRUCTION_QUARANTINE_STATUSES, AEGIS_INSTRUCTION_QUARANTINE_DECISIONS,
        AEGIS_INSTRUCTION_PATTERN_CLASSES, AEGIS_INSTRUCTION_QUARANTINE_SCENARIOS,
        ...
        AEGIS_ACTION_FIREWALL_STATUSES, AEGIS_ACTION_FIREWALL_DECISIONS,
        AEGIS_ACTION_KINDS, AEGIS_ACTION_FIREWALL_SCENARIOS,
        ...
        AEGIS_UI_PREFLIGHT_SOURCE_STATUSES, AEGIS_UI_PREFLIGHT_SURFACE_STATUSES,
        AEGIS_UI_PREFLIGHT_RENDERING_OUTCOMES, AEGIS_UI_PREFLIGHT_STATUS_MAPPINGS,
        AEGIS_UI_PREFLIGHT_SCENARIOS, AEGIS_UI_PREFLIGHT_REPRO_FRAGMENTS,
        AEGIS_UI_PREFLIGHT_ACCESSIBILITY_LANGUAGE, AEGIS_UI_PREFLIGHT_SECURITY_LANGUAGE,
        AEGIS_UI_PREFLIGHT_DOCTRINE, AEGIS_UI_PREFLIGHT_SOURCE_INTEGRITY_LANGUAGE,
        AEGIS_UI_PREFLIGHT_HASH_LANGUAGE, AEGIS_UI_PREFLIGHT_GUARDRAILS,
        AEGIS_UI_PREFLIGHT_BLOCKED_CLAIMS, AEGIS_UI_PREFLIGHT_PRIOR_PHASE_RELATION,
        AEGIS_UI_PREFLIGHT_STATUS_SURFACE_ARTIFACTS,
    ):
        for phrase in phrase_group:
            assert phrase in pages
    assert AEGIS_SOURCE_SCOPE_CONSENT_CLAIM_ALLOWED in pages
    assert AEGIS_SOURCE_SCOPE_CONSENT_DECISION_VOCAB_REPAIR_CLAIM_ALLOWED in pages
    assert "Source-scope / consent decisions" in pages
    ...
    assert AEGIS_UI_PREFLIGHT_CLAIM_ALLOWED in pages
