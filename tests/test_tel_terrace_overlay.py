...
class TelTerraceOverlayAssetsTests(unittest.TestCase):
    def test_html_has_toggle_and_legend_markers(self) -> None:
        ...
        self.assertIn('toggle-navigation', html)
        self.assertIn("window.addEventListener('DOMContentLoaded'", html)
    ...
    def test_atlas_integration_wires_overlay_and_reset(self) -> None:
        ...
        self.assertIn("bindAgentTelemetryOverlayToggle('toggle-agent-telemetry')", js)
        self.assertIn('applyAgentTelemetryOverlay(window.cy, agentId)', js)
        self.assertIn('agent_telemetry_event_map', js)
        atlas_js = Path('atlas/atlas.js').read_text(encoding='utf-8')
        self.assertIn('navigation_state', atlas_js)
        self.assertIn('applyNavigationOverlay', atlas_js)
        self.assertIn('clearNavigationOverlay', atlas_js)
        self.assertIn('bindNavigationToggle', atlas_js)
        self.assertIn('toggleAgentTelemetry', atlas_js)
    ...
    def test_agent_telemetry_overlay_assets(self) -> None:
        ...
        self.assertIn('telemetry-novelty', js)
        self.assertIn('telemetry-contradiction', js)
        self.assertIn('applyAgentTelemetryOverlay', js)
        self.assertIn('clearAgentTelemetryOverlay', js)
        self.assertIn('bindAgentTelemetryOverlayToggle', js)
        self.assertIn('applyAgentTelemetryOverlay(cy, agentId)', js)
        self.assertIn('agent_telemetry_event_map', js)
        atlas_js = Path('atlas/atlas.js').read_text(encoding='utf-8')
        self.assertIn('navigation_state', atlas_js)
        self.assertIn('applyNavigationOverlay', atlas_js)
        ...
        self.assertIn('bindNavigationToggle', atlas_js)
        self.assertIn('toggleAgentTelemetry', atlas_js)
        self.assertIn("bindAgentTelemetryOverlayToggle('toggle-agent-telemetry')", Path('atlas/atlas.js').read_text(encoding='utf-8'))
        css = Path('atlas/styles.css').read_text(encoding='utf-8')
        self.assertIn('.node.telemetry-novelty', css)
        self.assertIn('.node.telemetry-contradiction', css)
        self.assertIn('.cy .nav-psi-high', css)
        self.assertIn('.cy .nav-risk-high', css)
