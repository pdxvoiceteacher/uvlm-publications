...
def test_claim_validator_rejects_language_governance_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_visual_review_static_html_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
        "reproducibility_appendix.md",
        "claim_boundary_table.md",
        "artifact_table.md",
...
def test_claim_validator_allows_visual_review_static_html_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_visual_review_model_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
...
def test_claim_validator_allows_visual_review_model_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_language_governance_audit_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
...
def test_claim_validator_allows_language_governance_audit_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_allows_language_governance_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_static_html_usability_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
...
def test_claim_validator_allows_static_html_usability_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_static_html_usability_revision_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
    ...
    for claim in STATIC_HTML_USABILITY_REVISION_BLOCKED_CLAIMS:
        (paper_root / "PUB_GOV_ARTIFACT_COG_01.md").write_text(
            base + "\nThis publication overclaim says: " + claim, encoding="utf-8"
        )
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
...
def test_claim_validator_allows_static_html_usability_revision_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_ai_receipt_architecture_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
...
def test_claim_validator_allows_ai_receipt_architecture_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_validation_tiering_provenance_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
    ...
    for claim in VALIDATION_TIERING_PROVENANCE_BLOCKED_CLAIMS:
        (paper_root / "PUB_GOV_ARTIFACT_COG_01.md").write_text(
            base + "\nThis publication overclaim says: " + claim, encoding="utf-8"
        )
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
...
def test_claim_validator_allows_validation_tiering_provenance_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_telemetry_aperture_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
...
def test_claim_validator_allows_telemetry_aperture_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_tac_policy_simulation_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
    ...
    for claim in TAC_POLICY_SIMULATION_BLOCKED_CLAIMS:
        (paper_root / "PUB_GOV_ARTIFACT_COG_01.md").write_text(
            base + "\nThis publication overclaim says: " + claim, encoding="utf-8"
        )
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
...
def test_claim_validator_allows_tac_policy_simulation_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_tac_local_review_integration_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
    ...
    for claim in TAC_LOCAL_REVIEW_INTEGRATION_BLOCKED_CLAIMS:
        (paper_root / "PUB_GOV_ARTIFACT_COG_01.md").write_text(
            base + "\nThis publication overclaim says: " + claim, encoding="utf-8"
        )
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
...
def test_claim_validator_allows_tac_local_review_integration_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_tac_ai_receipt_event_link_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
    ...
    for claim in TAC_AI_RECEIPT_EVENT_LINK_BLOCKED_CLAIMS:
        (paper_root / "PUB_GOV_ARTIFACT_COG_01.md").write_text(
            base + "\nThis publication overclaim says: " + claim, encoding="utf-8"
        )
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
...
def test_claim_validator_allows_tac_ai_receipt_event_link_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_pmr_pathway_priors_design_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
    ...
    for claim in PMR_PATHWAY_PRIORS_DESIGN_BLOCKED_CLAIMS:
        (paper_root / "PUB_GOV_ARTIFACT_COG_01.md").write_text(
            base + "\nThis publication overclaim says: " + claim, encoding="utf-8"
        )
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
...
def test_claim_validator_allows_pmr_pathway_priors_design_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_coherence_event_signatures_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
    ...
    for claim in CES_DESIGN_BLOCKED_CLAIMS:
        (paper_root / "PUB_GOV_ARTIFACT_COG_01.md").write_text(
            base + "\nThis publication overclaim says: " + claim, encoding="utf-8"
        )
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
...
def test_claim_validator_allows_coherence_event_signatures_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_ces_pmr_indexing_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
    ...
    for claim in CES_PMR_INDEXING_DESIGN_BLOCKED_CLAIMS:
        (paper_root / "PUB_GOV_ARTIFACT_COG_01.md").write_text(
            base + "\nThis publication overclaim says: " + claim, encoding="utf-8"
        )
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
...
def test_claim_validator_allows_ces_pmr_indexing_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_claim_validator_rejects_triadic_observation_contract_overclaims(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    base = "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\nrequires external peer review\nnot AI consciousness\nnot Omega detection\nnot live Atlas memory writes\n"
    for name in (
    ...
    for claim in TRIADIC_OBSERVATION_CONTRACT_BLOCKED_CLAIMS:
        (paper_root / "PUB_GOV_ARTIFACT_COG_01.md").write_text(
            base + "\nThis publication overclaim says: " + claim, encoding="utf-8"
        )
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
...
def test_claim_validator_allows_triadic_observation_contract_bounded_claim(tmp_path):
    paper_root = tmp_path / "paper"
    paper_root.mkdir(parents=True)
    allowed = (
        "not truth certification\nnot deployment authority\nnot final answer release\nlocal fixture only\n"
...
def test_publication_claim_validator_rejects_mvr_readability_review_seed_overclaims(tmp_path):
    for claim in MVR_READABILITY_REVIEW_SEED_BLOCKED_CLAIMS:
        paper_root = tmp_path / claim.replace(" ", "_").replace("/", "_")
        _write_minimal_publication_claim_fixture(paper_root, f"MVR readability review seed claims {claim}.")
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
        assert result["forbidden_overclaims_found"], claim
...
def test_publication_claim_validator_rejects_gateway_scope_and_source_corpus_overclaims(tmp_path):
    for claim in GATEWAY_SCOPE_SOURCE_CORPUS_BLOCKED_CLAIMS:
        paper_root = tmp_path / claim.replace(" ", "_").replace("/", "_")
        _write_minimal_publication_claim_fixture(paper_root, f"Gateway scope source-corpus sync claims {claim}.")
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
        assert result["forbidden_overclaims_found"], claim


def test_publication_claim_validator_allows_bounded_gateway_scope_claim(tmp_path):
    paper_root = tmp_path / "paper"
    _write_minimal_publication_claim_fixture(paper_root, AI_RECEIPT_GATEWAY_SCOPE_SIMULATION_CLAIM_ALLOWED)
    result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
    assert result["forbidden_overclaims_found"] == []


def test_publication_claim_validator_allows_bounded_gateway_source_corpus_claims(tmp_path):
    for claim in (
        SOURCE_CORPUS_GATEWAY_REPORT_BATCH_CLAIM_ALLOWED,
        SOURCE_CORPUS_GATEWAY_REPORT_SOURCE_IDENTITY_REPAIR_CLAIM_ALLOWED,
    ):
...
def test_publication_claim_validator_rejects_gateway_local_ingress_overclaims(tmp_path):
    for claim in AI_RECEIPT_GATEWAY_LOCAL_INGRESS_BLOCKED_CLAIMS:
        paper_root = tmp_path / claim.replace(" ", "_").replace("/", "_")
        _write_minimal_publication_claim_fixture(paper_root, f"Local ingress sync claims {claim}.")
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
        assert result["forbidden_overclaims_found"], claim


def test_publication_claim_validator_allows_bounded_gateway_local_ingress_claim(tmp_path):
    paper_root = tmp_path / "paper"
...
def test_publication_claim_validator_rejects_control_package_registry_overclaims(tmp_path):
    for claim in CONTROL_PACKAGE_REGISTRY_BLOCKED_CLAIMS:
        paper_root = tmp_path / claim.replace(" ", "_").replace("/", "_")
        _write_minimal_publication_claim_fixture(paper_root, f"Control package registry sync claims {claim}.")
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
        assert result["forbidden_overclaims_found"], claim
...
def test_publication_claim_validator_rejects_catalog_bundle_overclaims(tmp_path):
    for claim in CONTROL_PACKAGE_CATALOG_BUNDLE_BLOCKED_CLAIMS:
        paper_root = tmp_path / claim.replace(" ", "_").replace("/", "_")
        _write_minimal_publication_claim_fixture(paper_root, f"Control package catalog bundle sync claims {claim}.")
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
        assert result["forbidden_overclaims_found"], claim


def test_publication_claim_validator_rejects_pricing_release_overclaims(tmp_path):
    for claim in PRICING_RELEASE_BLOCKED_CLAIMS:
        paper_root = tmp_path / claim.replace(" ", "_").replace("/", "_")
        _write_minimal_publication_claim_fixture(paper_root, f"Pricing release source sync claims {claim}.")
        result = validate_publication_claims(paper_root / "PUB_GOV_ARTIFACT_COG_01.md")
        assert result["forbidden_overclaims_found"], claim


def test_publication_claim_validator_allows_bounded_catalog_pricing_claims(tmp_path):
    for claim in (
        CONTROL_PACKAGE_CATALOG_BUNDLE_CLAIM_ALLOWED,
        PRICING_RELEASE_CLAIM_ALLOWED,
        PRICING_RELEASE_SCHEMA_REPAIR_CLAIM_ALLOWED,
...
def test_claim_validator_rejects_receipt_chain_export_overclaims():
    for claim in AEGIS_RECEIPT_CHAIN_BLOCKED_CLAIMS:
        assert claim in PAPER_CONFIGS["PUB-GOV-ARTIFACT-COG-01"]["forbidden_overclaims"]


def test_bounded_receipt_chain_export_claim_is_allowed():
    normalized = AEGIS_RECEIPT_CHAIN_CLAIM_ALLOWED.lower()
    for claim in AEGIS_RECEIPT_CHAIN_BLOCKED_CLAIMS:
        assert claim.lower() not in normalized


def test_claim_validator_rejects_local_runtime_enforcement_overclaims():
